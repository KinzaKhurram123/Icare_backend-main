...
exports.AddInstructorDetails = async (req, res) => {
    try {
        ...
        const existingProfile = await Instructor.findOne({ user: userId }).populate('user', 'name email phoneNumber role createdAt');
        if (existingProfile) {
...
exports.getAllInstructors = async (req, res) => {
    try {
        const instructors = await Instructor.find().populate('user', 'name email role phoneNumber');
...
exports.getMyInstructorProfile = async (req, res) => {
    try {
        const userId = req.user._id;
        const instructor = await Instructor.findOne({ user: userId }).populate('user', 'name email phoneNumber role createdAt');
        if (!instructor) {
...
exports.getInstructorById = async (req, res) => {
    try {
        const { id } = req.params;
        const instructor = await Instructor.findById(id).populate('user', 'name email role phoneNumber createdAt');
        if (!instructor) {
